...
def run_hmmer(input_file, cpus, evalue):
	...
	cmd = "hmmsearch -E "+ str(evalue) +" --cpu "+ cpus +" --tblout "+ output_file +" /home/frankaylward/viralrecall_dev/viralrecall/hmm/vogdb.hmm "+ input_file	
	print(cmd)
	cmd2 = shlex.split(cmd)
	#subprocess.call(cmd2, stdout=open("out.txt", "w"), stderr=open("err.txt", "w"))
	#os.remove("out.txt")
...
for i in os.listdir(inputdir):
	if i.endswith(".faa"):
		name1 = re.sub("_genomic.fna.faa", "", i)
		name = re.sub(".faa", "", name1)
		inputfile = os.path.join(inputdir, i)
		print(inputfile)
		#protein_file = predict_proteins(inputfile, inputdir)
		hmmout = run_hmmer(inputfile, "4", "1e-5")
